...
bool path_is_inside(const fs::path& child, const fs::path& parent) {
    std::error_code ec;
    const fs::path normalized_child = fs::weakly_canonical(child, ec);
    const std::string child_str = (ec ? child.lexically_normal() : normalized_child).string();
    ec.clear();
    const fs::path normalized_parent = fs::weakly_canonical(parent, ec);
    std::string parent_str = (ec ? parent.lexically_normal() : normalized_parent).string();
    ...
    return child_str == parent_str.substr(0, parent_str.size() - 1) ||
           child_str.rfind(parent_str, 0) == 0;
}

std::string first_lines(const std::string& text, int max_lines = 16) {
...
bool cpu_engine_links_rocm(const std::string& executable, std::string& details) {
    std::ostringstream output;
    const int code = ProcessManager::run_process_with_output(
        "ldd",
        {executable},
    ...
    details = output.str();
    if (code != 0 && details.empty()) {
        return false;
    }

    const std::string lowered = lowercase_copy(details);
    return lowered.find("libamdhip64") != std::string::npos ||
           lowered.find("libhiprtc") != std::string::npos ||
           lowered.find("libroc") != std::string::npos ||
           lowered.find("libhsa-runtime") != std::string::npos;
}

void verify_cpu_engine_binary(std::string& executable, BackendManager* backend_manager) {
    std::string rocm_details;
    if (!cpu_engine_links_rocm(executable, rocm_details)) {
        LOG(DEBUG, kLog) << "Verified lemon-mlx CPU engine has no ROCm/HIP dynamic dependency: "
                         << executable << std::endl;
        return;
    }

    const fs::path cpu_install_dir = BackendUtils::get_install_directory(kRecipe, "cpu");
    if (path_is_inside(executable, cpu_install_dir)) {
        LOG(WARNING, kLog)
            << "lemon-mlx CPU install contains a ROCm/HIP-linked engine; reinstalling CPU backend. Path: "
            << executable << std::endl;
        std::error_code ec;
        fs::remove_all(cpu_install_dir, ec);
        if (ec) {
            throw std::runtime_error("Failed to remove stale lemon-mlx CPU install at " +
                                     cpu_install_dir.string() + ": " + ec.message());
        }
        backend_manager->install_backend(kRecipe, "cpu");
        executable = BackendUtils::get_backend_binary_path(MlxServer::SPEC, "cpu");
        rocm_details.clear();
        if (!cpu_engine_links_rocm(executable, rocm_details)) {
            LOG(INFO, kLog) << "Reinstalled clean lemon-mlx CPU backend: " << executable << std::endl;
            return;
        }
    }

    throw std::runtime_error(
        "lemon-mlx CPU backend resolved to a ROCm/HIP-linked engine binary. "
        "A true CPU backend must not depend on ROCm/HIP and cannot execute hip_kernel. "
        "Check the cpu_bin override, clear the lemon-mlx/cpu install directory, or publish a corrected CPU engine asset. "
        "Executable: " + executable + "\nldd output:\n" + first_lines(rocm_details));
}
#endif
...
std::optional<int> positive_int_field(const json& object, const char* key) {
    if (!object.contains(key) || object[key].is_null()) {
        return std::nullopt;
    }
    if (!object[key].is_number_integer() && !object[key].is_number_unsigned()) {
        return std::nullopt;
    }
    const int value = object[key].get<int>();
    return value > 0 ? std::optional<int>(value) : std::nullopt;
}

void normalize_generation_limits(json& request) {
    const auto max_completion_tokens = positive_int_field(request, "max_completion_tokens");
    const auto max_tokens = positive_int_field(request, "max_tokens");

    if (max_completion_tokens && !max_tokens) {
        request["max_tokens"] = *max_completion_tokens;
    } else if (!max_tokens && request.contains("max_tokens")) {
        request.erase("max_tokens");
    }
}

void record_mlx_telemetry(json& response, double elapsed_seconds, MlxServer& server, int prompt_token_fallback = 0) {
    ...
    server.set_telemetry(telemetry.input_tokens,
                         telemetry.output_tokens,
                         telemetry.time_to_first_token,
                         telemetry.tokens_per_second);
    if (telemetry.input_tokens > 0) {
        server.set_prompt_tokens(telemetry.input_tokens);
    }
}
        ...
        if (chat_response && choice.contains("message") && choice["message"].is_object()) {
            ...
            if (!content.empty()) {
                ...
                chunks.push_back(std::move(content_chunk));
                   ...
                   !choice["text"].get<std::string>().empty()) {
            json chunk = {
...
std::string stream_error_message(const json& chunk) {
    ...
    if (error.is_object()) {
        ...
        if (error.contains("type") && error["type"].is_string()) {
            return error["type"].get<std::string>();
        }
        return error.dump();
    }
    return error.dump();
...
std::string stream_error_event(const std::string& message, const std::string& type) {
    json error = {
        {"error", {
            {"message", message},
            {"type", type}
...
void MlxServer::load(const std::string& model_name,
    ...
    port_ = choose_port();
    std::string executable = BackendUtils::get_backend_binary_path(SPEC, backend);
#if defined(__linux__)
    if (backend == "cpu") {
        verify_cpu_engine_binary(executable, backend_manager_);
    }
#endif
    ...
    if (backend == "cpu") {
        // Safe with current engine releases: unknown flags are ignored. Newer
        // lemon-mlx-engine builds can use this to force MLX's default device to
        // CPU before model load.
        args.push_back("--cpu");
    }
    ...
    if (backend == "rocm") {
        ...
        // On ROCm-capable systems MLX may select a GPU default device even when
        // Lemonade intentionally launched the CPU backend. Hide accelerators and
        // set an explicit engine hint so CPU requests do not reach hip kernels
        // and fail with "No ROCm back-end".
        env_vars.push_back({"LEMON_MLX_FORCE_CPU", "1"});
        env_vars.push_back({"HIP_VISIBLE_DEVICES", ""});
        env_vars.push_back({"ROCR_VISIBLE_DEVICES", ""});
        env_vars.push_back({"GPU_DEVICE_ORDINAL", ""});
        env_vars.push_back({"CUDA_VISIBLE_DEVICES", ""});
        LOG(INFO, kLog) << "Configured CPU runtime environment for lemon-mlx; GPU devices hidden" << std::endl;
    }
#elif defined(__APPLE__)
    std::vector<fs::path> lib_paths;
    append_path(lib_paths, executable_dir);
...
json MlxServer::prepare_request(const json& request) const {
    ...
    normalize_generation_limits(modified);

    if (is_small_qwen_model(request_model_ref(modified, loaded_model_ref_))) {
        // Do not inject unsupported sampling knobs for tiny Qwen MLX variants.
        // Keep repetition handling local to the stream bridge instead.
        modified.erase("repetition_penalty");
        modified.erase("frequency_penalty");
        modified.erase("presence_penalty");
        modified.erase("logit_bias");
    }
...
json MlxServer::chat_completion(const json& request) {
    ...
    normalize_reasoning_response(response);
    record_mlx_telemetry(response, seconds_since(started), *this, estimate_prompt_tokens(prepared));
    return response;
}
...
void MlxServer::forward_streaming_request(const std::string& endpoint,
    ...
    const bool chat_stream = endpoint == "/v1/chat/completions";
    const bool completion_stream = endpoint == "/v1/completions";
    const bool supported_stream = chat_stream || completion_stream;
    if (!sse || !supported_stream) {
        WrappedServer::forward_streaming_request(endpoint, prepared_body, sink, sse, timeout_seconds);
        return;
    }
    ...
    const bool raw_passthrough_stream = device_type_ == DEVICE_CPU || is_small_qwen_model(stream_model_ref);
    const bool normalize_reasoning = chat_stream && !raw_passthrough_stream;
    ...
    bool backend_done_without_payload = false;
    std::string backend_stream_error;
    int estimated_output_tokens = 0;
    std::string event_buffer;
    MlxTelemetrySnapshot telemetry;
    ReasoningStreamNormalizer normalizer(
        normalize_reasoning && prefers_prefix_reasoning(prepared_request, loaded_model_ref_, false));
    ...
    const auto emit_blocking_json_fallback = [&](json response) -> bool {
        if (response.contains("error")) {
            LOG(ERROR, kLog) << "Backend returned error during empty-stream fallback: "
    ...
    const auto emit_blocking_response_fallback = [&](const std::string& body) -> bool {
        try {
            return emit_blocking_json_fallback(json::parse(body));
    ...
    const auto process_event = [&](const std::string& event) -> bool {
        ...
        while (std::getline(lines, line)) {
            ...
            if (payload == "[DONE]") {
                // The normalizer deliberately keeps a small suffix buffered so
                // split <think> tags and stop markers are not leaked. Very short
                // CPU/tiny-model answers may therefore have produced no outward
                // token before the backend sends [DONE]. Flush first, then decide
                // whether the stream was actually empty.
                if (normalize_reasoning && !flush_reasoning()) {
                    return false;
                }
                if (!saw_first_token && estimated_output_tokens <= 0) {
                    backend_done_without_payload = true;
                    return true;
                }
                return send_final_done();
            }
            ...
            try {
                ...
                if (chunk.contains("error")) {
                    backend_stream_error = stream_error_message(chunk);
                    if (backend_stream_error.empty()) {
                        backend_stream_error = chunk.dump();
                    }
                    LOG(ERROR, kLog) << "Backend streaming error: " << backend_stream_error << std::endl;
                    const std::string error_msg = stream_error_event(backend_stream_error, "backend_stream_error");
                    sink.write(error_msg.c_str(), error_msg.size());
                    stream_error = true;
                    return false;
                }
    ...
    if (!stream_error) {
        ...
        if (!saw_first_token && estimated_output_tokens <= 0 && !client_aborted && !locally_finished) {
            const bool can_retry_blocking = device_type_ != DEVICE_CPU;
            LOG(INFO, kLog) << (backend_done_without_payload
                ? "MLX stream sent [DONE] without content"
                : "MLX stream ended without content")
                << (can_retry_blocking ? "; retrying once without streaming" : "; CPU blocking fallback skipped")
                << std::endl;

            if (can_retry_blocking) {
                json fallback_request = prepared_request;
                fallback_request["stream"] = false;
                fallback_request.erase("stream_options");
                normalize_generation_limits(fallback_request);
                if (emit_blocking_json_fallback(forward_request(endpoint, fallback_request, timeout_seconds))) {
                    // Fallback succeeded and already sent the terminal chunk.
                } else {
                    stream_error = true;
                }
            } else {
            ...
            if (stream_error) {
                const std::string message = backend_stream_error.empty()
                    ? "lemon-mlx produced no content before the first token"
                    : backend_stream_error;
                const std::string error_msg = stream_error_event(message,
                    backend_stream_error.empty() ? "empty_stream" : "backend_stream_error");
                sink.write(error_msg.c_str(), error_msg.size());
                sink.done();
            }
        }

        if (!stream_error && !has_done_marker) {
            send_final_done();
        }
        ...
        set_telemetry(telemetry.input_tokens,
                      ...
                      telemetry.tokens_per_second);
        if (telemetry.input_tokens > 0) {
            set_prompt_tokens(telemetry.input_tokens);
        }
