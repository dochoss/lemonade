...
def test_lemon_mlx_static_integration_contract():
    ...
    assert 'normalize_generation_limits' in mlx_server
    assert 'positive_int_field' in mlx_server
    assert 'emit_blocking_response_fallback' in mlx_server
    assert 'CPU blocking fallback skipped' in mlx_server
    assert 'const bool can_retry_blocking = device_type_ != DEVICE_CPU' in mlx_server
    assert 'raw_passthrough_stream' in mlx_server
    assert 'LEMON_MLX_FORCE_CPU' in mlx_server
    assert 'HIP_VISIBLE_DEVICES' in mlx_server
    assert 'verify_cpu_engine_binary' in mlx_server
    assert 'cpu_engine_links_rocm' in mlx_server
    assert 'libamdhip64' in mlx_server
    assert 'ROCm/HIP-linked engine' in mlx_server
    assert 'ROCR_VISIBLE_DEVICES' in mlx_server
    assert 'args.push_back("--cpu")' in mlx_server
    assert 'device_type_ == DEVICE_CPU || is_small_qwen_model(stream_model_ref)' in mlx_server
    assert 'emit_blocking_json_fallback' in mlx_server
    assert 'lemon-mlx produced no content' in mlx_server
    assert 'request["stream"] = true' in mlx_server
    assert 'mlx-community/Qwen3.5-0.8B-8bit' in _read('src/cpp/resources/server_models.json')
    ui_chat = _read("src/app/src/renderer/components/panels/LLMChatPanel.tsx")
    assert "!accumulatedContent && !accumulatedThinking" in ui_chat
    assert "openThinkIndex" in ui_chat
    assert 'streamError' in ui_chat
    assert 'parsed.error' in ui_chat
...
def test_lemon_mlx_models_are_modern_and_sized():
    ...
    assert "Qwen3-0.6B-MLX" not in lemon_mlx_models
    assert "Gemma-4-31B-it-MLX" not in lemon_mlx_models
...
def test_readme_documents_lemon_mlx_without_overstating_support():
    ...
    assert '<td rowspan="2"><code>vllm</code> (experimental)</td>' in readme
